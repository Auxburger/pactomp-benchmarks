...
## Dynamic Resource Manager — NPB3.4-OMP Benchmarks on CoolMUC-4
...
| `NPB3.4-OMP/test_all.sh` | Removed `OMP_PROC_BIND`/`OMP_PLACES`; per-t taskset (`CPU_A_T`, `CPU_B_T`); `DRM_CPU_LIST` per t |
| `NPB3.4-OMP/test-one.sh` | Added `iter_start` parameter; parallelised iteration loop |
| `NPB3.4-OMP/test_staggered.sh` | New: staggered-start experiment with per-iteration timing |
| `NPB3.4-OMP/run_staggered.sbatch` | New: SLURM job for staggered experiment |
| `NPB3.4-OMP/build_omp.sh` | Fixed: module load failures silently killed script; added per-step logging |
