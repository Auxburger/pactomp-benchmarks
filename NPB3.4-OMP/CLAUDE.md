# CLAUDE.md — NPB3.4-OMP DRM Benchmark Suite
...
| What | Where |
|------|-------|
| DRM source (Rust) | `/dss/dsshome1/09/ga27qam2/dynamic-resource-manager/src/` |
| LLVM runtime source | `/dss/dsshome1/09/ga27qam2/llvm-project/openmp/runtime/src/kmp_resource_manager.cpp` |
| LLVM build output | `/dss/dsshome1/09/ga27qam2/llvm-project/build/lib/libomp.so` |
| NPB binaries | `bin/ft.C.x`, `bin/cg.C.x`, `bin/ep.C.x` |
| Benchmark results | `benchmarks/dual/`, `benchmarks/staggered/` |
| SLURM logs | `slurm_logs/slurm-<JOBID>.out/.err` |
...
**Output files** (in `benchmarks/staggered/`):
```
...
ldd bin/ft.C.x | grep omp  # confirms dynamic link to LLVM build
```
...
| `LLVM_BUILD` | `/dss/dsshome1/09/ga27qam2/llvm-project/build` | Used for `LD_LIBRARY_PATH` |
