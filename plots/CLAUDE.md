...
Run everything with:
```bash
cd src && python main.py           # newest staggered job only (fast, skips unchanged outputs)
cd src && python main.py --all     # all staggered jobs
cd src && python main.py --static  # also export PDFs next to each HTML (requires kaleido)
cd src && python main.py --png     # also export PNGs next to each HTML (requires kaleido)
```
...
| `src/plots/pipeline.py` | Groups dual/dual-exclusive run directories |
| `src/plots/parsing.py` | Parses NPB benchmark result logs (runtime, MOPS) |
| `src/plots/plotting.py` | Aggregated benchmark bar/line plots |
| `src/plots/monitoring_parsing.py` | Parses `rm.log` (DRM grants) and `pidstat_*.log` |
| `src/plots/monitoring_plots.py` | CPU placement Gantt, DRM allocation violin, slab assignment bar |
| `src/plots/cpu_util_parsing.py` | Parses `mpstat -P ALL` output (`cpu_util_*.log`) |
| `src/plots/cpu_util_plots.py` | Per-CPU utilisation heatmap (time × core) |
| `src/plots/staggered_parsing.py` | Parses staggered worker logs and groups them |
| `src/plots/staggered_plots.py` | Per-iteration duration scatter plot for staggered runs |
| `src/plots/io_utils.py` | `write_outputs()` — saves HTML, optionally PDF (`also_static=True`) and/or PNG (`also_png=True`, `scale=2`) |
| `export_thesis_figs.py` | Standalone script: generates thesis-quality PDFs for job 187303 into `../figures/` |
| `src/plots/scalability_model.py` | Amdahl--Karp--Flatt fit over `dual`: OLS effective fraction, pointwise Karp--Flatt, launch-group bootstrap, hold-out |
| `export_scalability_model.py` | Standalone script: writes the model CSVs to `plots/model/` and `amdahl_karp_flatt_capacity.pdf` into `../figures/` |
